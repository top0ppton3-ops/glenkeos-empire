...
import { Package, Truck, ShoppingCart, Clock, Users, BarChart3, Settings, LogOut, Bell, User } from "lucide-react";
import { useState, useEffect } from "react";

export function CorporateOperationsLayout() {
  const location = useLocation();
  const [user, setUser] = useState<any>(null);

  useEffect(() => {
    const userData = localStorage.getItem('corporate_user');
    if (userData) {
  ...
  const navigationItems = [
    ...
    {
      path: "/corporate/operations/orders",
      label: "Orders",
      icon: ShoppingCart
    },
    {
      path: "/corporate/operations/kitchen",
      label: "Kitchen (KDS)",
      icon: Clock
    },
    {
      path: "/corporate/operations/drivers",
      label: "Drivers",
      icon: Truck
    },
    {
      path: "/corporate/operations/inventory",
      label: "Inventory",
      icon: Package
    },
    {
      path: "/corporate/operations/staff",
      label: "Staff",
      icon: Users
    },
    {
      path: "/corporate/operations/settings",
      label: "Settings",
      icon: Settings
    }
  ];
  ...
  return (
    <div className="min-h-screen flex" style={{ backgroundColor: 'var(--b1-black-marble)' }}>
      ...
      >
        {/* Logo */}
        <div className="p-6 border-b" style={{ borderBottomColor: 'var(--b1-border-subtle)' }}>
          <Link to="/" className="flex items-center gap-3">
            <div className="w-10 h-10 border flex items-center justify-center" style={{ borderColor: 'var(--b1-gold-trim)' }}>
              <Package className="w-5 h-5" style={{ color: 'var(--b1-gold-trim)' }} />
            </div>
            <div>
        ...
        <header className="h-16 border-b flex items-center justify-between px-6" style={{ backgroundColor: 'var(--b1-obsidian)', borderBottomColor: 'var(--b1-border-subtle)' }}>
          <div>
            <h1 className="text-lg tracking-wider" style={{ fontWeight: 500 }}>
              {navigationItems.find(item => item.path === location.pathname)?.label || 'Operations'}
            </h1>
          </div>
